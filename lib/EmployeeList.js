...
class EmployeeList {
    ...
    async addEmployee(newEmployee) {
        //get id for manager
        let manager_id;
        if(newEmployee.employee_manager !== 'None') {
            manager_id = await this.db.query(`SELECT id FROM employees WHERE CONCAT(first_name, " ", last_name)="${newEmployee.employee_manager}"`);
    ...
    async deleteDept(dept) {
        let dept_id = await this.db.query(`SELECT id FROM departments WHERE title="${dept.title}"`);
        dept_id = Object.values(dept_id[0][0])[0];

        await this.db.query(`DELETE FROM departments WHERE id = ${dept_id}`);
